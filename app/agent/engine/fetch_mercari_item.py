...
class FetchMercariItem():
    ...
    def fetch_mercari_item(url:str, item_id:str):
        try:
            ...
            category = FetchMercariItem.find_table_th_get_td(table_th, table_td, "カテゴリー")
        ...
        return SearchedItem(item_name=item_name.text, item_id=item_id, description=description.text, 
                           thumbnail_url=thumbnail_url, image_urls=image_urls, category=category, brand=brand, condition=condition,
    ...
    def fetch_keyword_searched_item_urls(url:str):
        try:
            ...
            soup = bs(res_text,"html.parser")
            # ページが存在しない場合は空の配列を返す
            if soup.select_one(".search-result-description") != None:
                if soup.select_one(".search-result-description").text.find('該当する商品が見つかりません。') >= 0:
                    return []
    ...
    @staticmethod
    def fetch_keyword_searched_items(keyword:str, min_price:int=0, max_price:int=999999, max_page_num:int=1):
        items = []
        urls = []
        # 最大ページ数分をスクレピング
        for page in range(max_page_num):
            url = MERCARI_SEARCH_URL + "?" + FetchMercariItem.create_search_query(keyword,min_price,max_price,page)
            _urls = FetchMercariItem.fetch_keyword_searched_item_urls(url)
            # ページが存在しない場合は終了
            if len(_urls) == 0:
                break 
            urls.extend(_urls)
        for url in urls:
            item = FetchMercariItem.fetch_mercari_item(MERCARI_BASE_URL + url,url)
            if item != None:
                items.append(item)
